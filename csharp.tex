...
\begin{exerciseframe}{Events Are Not Fields}
  ...
  unsubscribe one of
  them, fire, and predict the output. Then try these two statements from
  \emph{outside} \texttt{Alarm} and read the error.
  \vspace{1ex}
...
\begin{solutionframe}{Events Are Not Fields}
  \begin{itemize}
    ...
    \item Both statements are rejected: \texttt{CS0070: The event
      'Alarm.Triggered' can only appear on the left hand side of += or -=
...
\begin{exerciseframe}{One Method Instead of Two}
  \texttt{SumOfOdds} differs from \texttt{SumOfEvens} in the comparison only.
...
int odds = Sum(numbers, n => n % 2 != 0);    // 4
  \end{lstlisting}
  \hint{Passing behaviour instead of duplicating loops is the very shape behind
  LINQ's \texttt{Where}.}
\end{solutionframe}

\begin{exerciseframe}{Behaviour as a Parameter}
  ...
  What do the two statements print?
  \vspace{1ex}
...
\begin{solutionframe}{Behaviour as a Parameter}
  \begin{lstlisting}[basicstyle=\scriptsize\ttfamily]
static bool Retry(int times, Func<bool> attempt)
{
