...
\begin{exerciseframe}{Reparent Without Moving}
    ...
    \begin{enumerate}
        ...
        \item The player avatars live in
        \texttt{Resources/Prefabs/Players/CC4}: \texttt{Caleb},
        \texttt{Hanna}, \texttt{Karl} and a dozen more, all built on
        \texttt{BaseAvatar}. A new character is needed --- same rig, same
        components, a different look. Variant or copy, and what happens to
        each when \texttt{BaseAvatar} later gains a component?
    \end{enumerate}
\end{exerciseframe}
...
\begin{solutionframe}{Reparent Without Moving}
    \begin{enumerate}
        ...
        \item A variant --- and that is what SEE does: fifteen of the sixteen
        character prefabs are variants of \texttt{BaseAvatar.prefab} and pick
        the new component up by themselves. \texttt{Assistant.prefab} is the
        exception, a standalone prefab, so somebody has to remember it.
    \end{enumerate}
\end{solutionframe}
