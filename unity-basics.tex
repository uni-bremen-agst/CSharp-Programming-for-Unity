...
\begin{exerciseframe}{Variant or Copy?}
  ...
  \begin{enumerate}
    ...
    \item One character stands too low in the scene. The fix could go on the
      scene instance, on the variant, or on \texttt{BaseAvatar}. Which one, and
      what does each choice mean for the other fifteen?
  \end{enumerate}
\end{exerciseframe}
...
\begin{solutionframe}{Variant or Copy?}
  \begin{enumerate}
    ...
    \item If only that character sits wrong, fix its variant. If all of them
      do, fix \texttt{BaseAvatar} and the fifteen variants follow --- but
      \texttt{Assistant} will not, and neither will any scene instance whose
      position somebody has already moved by hand, because that is an
      override.
  \end{enumerate}
\end{solutionframe}
